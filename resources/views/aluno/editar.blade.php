...
<div>
    ...
    <form class="form-prevent-mult-submits" method="POST" action="{{ route('aluno.update') }}" enctype="multipart/form-data">
                ...
                <div class="d-flex">
                    @if(Auth::user()->image)
                        <div class="circle img-perfil-pequena mr-3">
                            ​<img src="{{ asset('storage/perfil/users/' . Auth::user()->image) }}" alt="imagem de perfil">
                        </div>
                    @else
                        <div class="circle img-perfil-pequena mr-3">
        ...
        <button type="submit" class="mt-2 btn btn-primary btn-sm button-prevent-mult-submits" alt="botao-salvar-alteracoes" title="Salvar alterações">
